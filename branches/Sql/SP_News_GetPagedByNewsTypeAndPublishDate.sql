...
-- =============================================
CREATE PROCEDURE [dbo].[News_GetPagedByNewsTypeAndPublishDate]
     @NewsType INT ,
      @STime DATETIME,
   @ETime DATETIME,
    @PublishStatus INT ,
...
BEGIN	
	...
	BEGIN
			...
			WHERE NewsType=@NewsType	AND PublishDate>=@STime AND PublishDate<=@ETime
	...
	BEGIN
			...
			SELECT	NewsId  
			FROM dbo.newsinfo WITH(NOLOCK)	
			WHERE [Status] =@PublishStatus 	 AND NewsType=@NewsType	AND PublishDate>=@STime AND PublishDate<=@ETime
			ORDER BY PublishDate DESC 	
	END 
